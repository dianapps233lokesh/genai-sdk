...
class QAResponse(BaseModel):
    ...
    sources: List[str] = Field(description="List of sources used for this answer")
...
def get_conversational_chain(st_callback):
    ...
    model = ChatGoogleGenerativeAI(
        model="gemini-2.5-pro", streaming=True, callbacks=[st_callback]
    )
...
def user_input(user_question):
    ...
    agent_executor = get_agent_executor(
        tools=tools, prompt=prompt, st_callback=st_callback
    )

    response = agent_executor.invoke({"input": user_question})
    try:
        st.json(response["output"])  # nicely display in Streamlit
    except Exception as e:
        st.error(f"Parsing failed: {e}")
        st.write("Raw Output:", response["output"])
